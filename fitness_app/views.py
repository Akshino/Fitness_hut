...
class CreatePurchaseOrderView(View):
    ...
    def post(self, request):
        ...
        return redirect('/admin/fitness_app/purchaseorder/')  # Redirect to a success page
...
def visualization_view(request):
    # Query data for gender distribution (Bar Chart)
    gender_data = Product.objects.values('gender').annotate(count=models.Count('gender'))

    # Query data for brand popularity (Pie Chart)
    brand_data = Product.objects.values('brand').annotate(total_stock=models.Sum('stock'))

    # Query data for type distribution (Line Chart)
    type_data = Product.objects.values('type').annotate(count=models.Count('type'))
    ...
    type_data = json.dumps(list(type_data), cls=DjangoJSONEncoder)
...
# **IMPORTANT:** It's a best practice to store API keys in environment variables
# and not directly in your code. Consider using Django's settings or environment
# variables to manage sensitive information.
genai.configure(api_key="AIzaSyAEpxEaoSLL6Z6gBM3Ha0edMAECjW6h61g")

def fitness_supplement_recommendation(request):
    recommendation = None

    if request.method == "POST":
        try:
            # Retrieve and validate POST data
            height = float(request.POST.get('height'))  # Height in centimeters
            weight = float(request.POST.get('weight'))  # Weight in kilograms
            ...
            prompt = f"""
            Based on the following user details, recommend a suitable fitness supplement:
            ...
            **Requirements:**
            1. Supplement Name
            2. Description of how to use it
            3. Exercise tips to build a healthy body
            """
            ...
            # **Fetch and Process the Recommendation**
            recommendation = response.text.strip()

        except (ValueError, TypeError) as e:
            recommendation = "Invalid input. Please ensure all fields are filled out correctly."

    return render(request, 'fitness_recommendation.html', {
        'recommendation': recommendation,
        'bmi': bmi if 'bmi' in locals() else None  # Pass BMI to template if calculated
    })
...
PREDEFINED_RESPONSES = {
    ...
    "I have a problem with my order. Who should I contact?": "If you have any issues with your order, please contact our customer support team through the contact form on our website or by email at akshinocherian17@gmail.com.",
    ...
    "How can I contact customer support?": "You can contact our customer support through the contact form on our website or by emailing us at akshinocherian17@gmail.com.",
