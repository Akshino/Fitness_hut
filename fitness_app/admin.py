...
class ProductAdmin(admin.ModelAdmin):
    list_display = ['display_image','name', 'seller', 'subcategory', 'price', 'quantity_in_stock','brand','gender']
    list_filter = ['seller','subcategory','brand']
        ...
        for product in InventoryProduct.objects.all():
            if product.quantity_in_stock < product.reorder_level:
                ...
                send_mail(
                    ...
                    ['akshino.23pmc103@mariancollege.org'],  # Replace with admin email
                    fail_silently=False,
                )
